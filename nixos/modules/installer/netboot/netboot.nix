...

{
  ...
  imports = [
     (mkRemovedOptionModule [ "system" "build" "netbootRamdisk" ] "`system.build.netbootRamdisk` was a special initrd for netbooting, it is not necessary to use it as `system.build.initialRamdisk` contains everything you need now and is standard across nixpkgs.")
  ];
  ...
  config = {
    ...
    system.build.netbootIpxeScript = pkgs.writeTextDir "netboot.ipxe" ''
      ...
      kernel ${pkgs.stdenv.hostPlatform.linux-kernel.target} init=${config.system.build.toplevel}/init initrd=${config.system.build.initialRamdisk}/${config.system.boot.loader.initrdFile} ${toString config.boot.kernelParams} ''${cmdline}
